...
// API route: /api/* → API origin (direct, no caching — higher priority than /*)
resource apiSpaRoute 'Microsoft.Cdn/profiles/afdEndpoints/routes@2023-05-01' = {
  parent: endpoint
  name: 'route-api-spa'
  properties: {
    originGroup: {
...
resource webRoute 'Microsoft.Cdn/profiles/afdEndpoints/routes@2023-05-01' = {
  ...
  dependsOn: [
    webOrigin
    apiRoute       // /health route
    apiSpaRoute    // /api/* route — must exist before /* to ensure correct priority
  ]
}
